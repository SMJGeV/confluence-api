 ...
 * @param {Function} callback
 */
Confluence.prototype.getContentChildByContentId = function(id, options, callback) {
    ...
    if (Array.isArray(options.params) && options.params.length) {
 	...
 	request
		.get(
			this.config.baseUrl +
				...
				id +
				"/child" +
                (typeof options.childtype === "string" ? "/" + options.childtype : "") +
                "?expand=" + expanders.join() +
                params
		)
		.auth(this.config.username, this.config.password)
		.end(function(err, res) {
